...
import { xlayerChain, wagmiConfig } from "@/lib/chain";
import { PAYPORT_ADDRESS } from "@/lib/contract-address";
import { isUnknownLinkError } from "@/lib/payport-errors";
...
export function assetSymbol(asset?: SettlementAsset) {
...
export function formatAssetAmount(
  ...
  return Number(formatUnits(value, USDC.decimals)).toLocaleString("en-US", {
 ...
 * refunded to the payer in the same transaction.
 */
export function usePayLink(slug: string) {
  ...
  const {
    ...
    args: [address ?? "0x0000000000000000000000000000000000000000", PAYPORT_ADDRESS],
    chainId: xlayerChain.id,
    query: {
  ...
  const pay = useCallback(
    async (requiredToken: bigint) => {
      const value = withQuoteBuffer(requiredToken);
      resetPay();
      resetApprove();
      setHash(undefined);
...
export function usePaymentReceipt(slug: string, txHash: `0x${string}` | undefined) {
  const client = usePublicClient({ chainId: xlayerChain.id });
  ...
  const block = useQuery({
    queryKey: ["payport-receipt-block", txHash, String(receipt.data?.blockNumber ?? 0n)],
  ...
  const payment =
    validation.status === "valid"
      ? ({
          ...
          usdPrice: validation.event.args.feedUsdPrice,
          usdDecimals: Number(validation.event.args.feedUsdDecimals),
          paidAt: block.data?.timestamp ?? 0n,
          hash: txHash as `0x${string}`,
          blockNumber: receipt.data?.blockNumber ?? 0n,
          asset: 1 as SettlementAsset,
 ...
 * until the latest few for this merchant are found.
 */
  ...
  while (offset < total && payments.length < collectLimit && pagesRead < maxPages) {
    ...
    for (let index = 0; index < rawPayments.length && payments.length < collectLimit; index++) {
      ...
      payments.push({
        ...
        usdPrice: payment.feedUsdPrice,
        usdDecimals: Number(payment.feedUsdDecimals),
        asset: 1 as SettlementAsset,
      });
    }
...
/** Formats a fixed USD price value and its decimals into a readable rate. */
...
export function withQuoteBuffer(requiredToken: bigint) {
  return requiredToken + (requiredToken * QUOTE_BUFFER_BPS) / BPS;
}
