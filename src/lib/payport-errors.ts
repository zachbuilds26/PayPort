...
export function formatReadError(error: unknown, fallback = "PayPort could not load the network right now. Try again in a moment.") {
...
export function formatWriteError(
  ...
  if (/insufficient funds/i.test(text)) {
    return "This wallet needs more testnet OKB for gas.";
  }
