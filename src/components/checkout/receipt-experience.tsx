...
import { explorerTx, xlayerChain, shortenAddress } from "@/lib/chain";
...
export function ReceiptExperience({
  ...
  const { link, isLoading: linkLoading, error: linkError, notFound } = usePaymentLink(slug);
  const receipt = usePaymentReceipt(slug, txHash);
  const storedReceipts = useStoredReceipts();

  const stored = useMemo<StoredReceipt | undefined>(() => {
    if (txHash) {
      return storedReceipts.find((r) => r.slug === slug && r.hash === txHash);
    }
    return storedReceipts.find((r) => r.slug === slug);
  }, [storedReceipts, slug, txHash]);

  const resolvedHash = txHash ?? stored?.hash;
    ...
    return (
      <ReceiptShell>
        <Issue
          ...
          body="Open the receipt link with its ?tx= transaction hash, or check your receipts history."
        />
      </ReceiptShell>
    );
  }
  ...
  if (receipt.payment) {
    return (
      <ReceiptShell>
        <ReceiptCard
          title={link?.title ?? stored?.title ?? "Payment"}
          priceUsdCents={receipt.payment.priceUsdCents}
          amountWei={receipt.payment.amountWei}
          usdPrice={receipt.payment.usdPrice}
          usdDecimals={receipt.payment.usdDecimals}
  ...
  if (linkLoading || receipt.isLoading) {
    return (
      <ReceiptShell>
  ...
  if (stored && !receipt.payment && !receipt.isLoading) {
    return (
      <ReceiptShell>
        <StoredReceiptCard stored={stored} link={link} />
      </ReceiptShell>
    );
  }

  if ((linkError && !notFound) || receipt.error) {
    return (
      <ReceiptShell>
        <Issue
          title="We could not load this receipt."
          body={formatReadError(
            receipt.error ?? linkError,
            "PayPort could not verify this transaction right now. Try again in a moment.",
          )}
        />
      </ReceiptShell>
    );
...
function StoredReceiptCard({
  stored,
  link,
}: {
  stored: StoredReceipt;
  link?: { title: string; priceUsdCents: bigint } | null;
}) {
  const title = link?.title ?? stored.title;
  const priceCents = link?.priceUsdCents ?? BigInt(stored.priceUsdCents);
  const symbol = assetSymbol(1);
  const paidAtLabel = new Date(stored.paidAt * 1000).toLocaleString();
    ...
    <section className="border border-line bg-surface">
      ...
      <dl className="divide-y divide-line px-5 text-sm sm:px-7">
        <div className="flex items-center justify-between gap-4 py-4">
          ...
          <dd className="font-semibold">{xlayerChain.name}</dd>
        </div>
        <div className="flex items-center justify-between gap-4 py-4">
...
function ReceiptCard({
  ...
  const rateLabel = `${formatFeedPrice(usdPrice, usdDecimals)} USD`;
  ...
  async function downloadJpeg() {
    ...
    try {
      ...
      const fileName = `payport-receipt-${
        title
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
      ...
      <div className="border border-line bg-surface text-ink">
        ...
        <div className="px-5 py-5 sm:px-7">
          ...
          <p className="mt-4 text-[10px] leading-5 text-faint">
            Verified on {xlayerChain.name}. Amounts and rate are set onchain at payment time.
          </p>
        </div>
      </div>
    </div>
